...
function New-AzDataProtectionRestoreConfigurationClientObject{
    ...
    param(
        ...
        [Microsoft.Azure.PowerShell.Cmdlets.DataProtection.Models.Api202301.KubernetesClusterRestoreCriteriaNamespaceMappings] # DppRef: change type
        ${NamespaceMapping},
    ...
    process {
        ...
        # DppRef: validate by just fetching the objects if the conditional is working fine
        ...
        # DppRef: update psd1 file
        # DppRef: add mapping
        # if($NamespaceMapping -ne $null -and $NamespaceMapping.Count -gt 0){
        #    Write-Host ($NamespaceMapping | fl)            
        #}
        #else {
        #    Write-Host "null mapping"
        #}
