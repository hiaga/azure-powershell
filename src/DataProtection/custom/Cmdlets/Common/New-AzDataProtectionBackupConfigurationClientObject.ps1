...
function New-AzDataProtectionBackupConfigurationClientObject{
    ...
    process {        
        ...
        if($DatasourceType.ToString() -eq "AzureKubernetesService"){
            ...
            if($VaultedBackupContainers -ne $null -or $IncludeAllContainers){
                $message = "Invalid parameter VaultedBackupContainers or IncludeAllContainers for given DatasourceType."
                throw $message
            }
        ...
        if($DatasourceType.ToString() -eq "AzureBlob"){
            ...
            if($VaultedBackupContainers -ne $null){
                $dataSourceParam.ContainersList = $VaultedBackupContainers
            }
            elseif($IncludeAllContainers){
            ...
            elseif($ExcludedResourceType -ne $null -or $IncludedResourceType -ne $null -or $ExcludedNamespace -ne $null -or $IncludedNamespace -ne $null -or $LabelSelector -ne $null -or $SnapshotVolume -ne $null -or $IncludeClusterScopeResource -ne $null){
                ...
                throw $message
                 ...
                 $message = "Please input VaultedBackupContainers or IncludeAllContainers parameters for given workload type."
                 throw $message
            }
        }
